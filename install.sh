...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY SAMZ OFFC            [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "samzoffc" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di SamzMarket${NC}"
    echo -e "${YELLOW}TELEGRAM : @SamzOffc${NC}"
    echo -e "${YELLOW}WHATSAPP : +6285136825418${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© SamzOffc${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  ...
  THEME_URL="https://github.com/samzmarket/Auto-install/raw/main/${THEME_NAME}.zip"
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/samzmarket/Auto-install/raw/main/stellar.zip
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer SamzOffc   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : SamzOffc      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : SamzOffc  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by SamzOffc  ${NC}"
